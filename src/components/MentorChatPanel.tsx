...
import type { ChatMessage, CalendarTask, Note, ProjectMeta, MentorChatMode } from '@/lib/types';
...
interface MentorChatPanelProps {
  ...
  author: string;
  chatMode: MentorChatMode;
  onDocAttached: (text: string, name: string) => void;
  onClearAttachments: () => void;
  onMessagesChange: (updater: (prev: ChatMessage[]) => ChatMessage[]) => void;
  onCommitPlan: () => void;
  onNoteCreated: () => void;
  onModeChange: (mode: MentorChatMode) => void;
  onClose: () => void;
}

const MODES: { id: MentorChatMode; label: string; icon: any; color: string }[] = [
  { id: 'mentor', label: 'Mentor', icon: GraduationCap, color: '#8b5cf6' },
  { id: 'literature', label: 'Literature', icon: BookOpen, color: '#3b82f6' },
  { id: 'methodology', label: 'Methodology', icon: Microscope, color: '#10b981' },
...
export default function MentorChatPanel({
  ...
  const handleMouseDownResize = useCallback(
    (e: React.MouseEvent) => {
      ...
      const onMouseMove = (ev: MouseEvent) => {
        setAiPanelWidth(Math.max(350, Math.min(800, startWidth + (startX - ev.clientX))));
      };
      const onMouseUp = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await fetch('/api/research/mentor_chat', {
        method: 'POST',
  ...
  const handleSaveToNote = async (content: string, index: number) => {
    ...
    if (!folder) return;
    const filename = prompt('Enter filename for the new note:', `Mentor_Response_${new Date().getTime()}.md`);
    if (!filename) return;
    ...
    try {
      const res = await fetch('/api/notes/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ folderPath: folder, filename, content, author }),
      });
      if (res.ok) { onNoteCreated(); alert('Note saved successfully!'); }
      else { const data = await res.json(); alert(`Failed to save note: ${data.error}`); }
  ...
  const handleDownload = (content: string) => {
    ...
    a.href = url;
    a.download = `Mentor_Response_${new Date().toISOString().replace(/[:.]/g, '-')}.md`;
  ...
  const injectSource = (relPath: string, summary?: string) => {
    const text = summary ? `Re: "${relPath}" — ${summary}` : `Tell me more about "${relPath}"`;
  ...
  return (
    ...
    >
      <div className={styles.aiPanelResizeHandle} onMouseDown={handleMouseDownResize}><GripVertical size={12} /></div>

      <div className={styles.aiPanelHeader}>
        <div className={styles.aiPanelTitle}>
          <div className={styles.aiPanelTitleIcon} style={{ background: `${currentModeInfo.color}40`, color: currentModeInfo.color }}>
            <currentModeInfo.icon size={16} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ fontWeight: 700 }}>{currentModeInfo.label} Mode</span>
            <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>Research Mentor</span>
          </div>
        </div>
        <button className={styles.aiPanelClose} onClick={onClose}><X size={16} /></button>
      </div>
      ...
      <div style={{ 
        ...
        {MODES.map(m => {
          ...
          return (
            ...
            >
              <m.icon size={14} />
              <span style={{ fontSize: '0.6rem', fontWeight: isActive ? 700 : 500 }}>{m.label}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className={styles.aiPanelContent}>
        ...
        {indexedSources.length > 0 && (
          <div style={{ border: '1px solid rgba(16,185,129,0.2)', borderRadius: '6px', marginBottom: '10px', overflow: 'hidden' }}>
            <button
              onClick={() => setShowSources((v) => !v)}
              style={{ display: 'flex', alignItems: 'center', gap: '6px', width: '100%', padding: '7px 10px', background: 'rgba(16,185,129,0.07)', border: 'none', cursor: 'pointer', color: '#10b981', fontSize: '0.77rem' }}
            >
              <Database size={12} /><span style={{ flex: 1, textAlign: 'left', fontWeight: 600 }}>Project Sources ({indexedSources.length})</span>
            ...
            {showSources && (
              <div style={{ padding: '6px 8px', display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '180px', overflowY: 'auto' }}>
                {indexedSources.map((src, i) => (
                  <button key={i} onClick={() => injectSource(src.relativePath, src.summary)} style={{ display: 'flex', alignItems: 'center', gap: '6px', textAlign: 'left', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '4px', padding: '5px 8px', cursor: 'pointer', width: '100%' }}>
                    <span style={{ fontSize: '0.6rem', padding: '1px 4px', borderRadius: '3px', flexShrink: 0, background: src.fileType === 'pdf' ? 'rgba(239,68,68,0.15)' : 'rgba(59,130,246,0.15)', color: src.fileType === 'pdf' ? '#ef4444' : '#3b82f6' }}>{src.fileType.toUpperCase()}</span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{src.relativePath}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {summaryContent && (
          <div className={styles.aiSummarySection}>
            <div className={styles.aiSectionLabel}><Sparkles size={12} /><span>Summary</span></div>
            <div className={styles.aiSummaryBody}><MarkdownRenderer content={summaryContent} /></div>
          </div>
        )}
        ...
        {chatMessages.length > 0 && (
          <div className={styles.aiChatSection}>
            <div className={styles.aiSectionLabel}><MessageCircle size={12} /><span>{currentModeInfo.label} Conversation</span></div>
      ...
      <div className={styles.aiPanelFooter}>
        {attachedDocsNames.length > 0 && <div style={{ fontSize: '0.75rem', color: 'var(--accent-base)', marginBottom: '8px' }}>📎 Attached: {attachedDocsNames.join(', ')}</div>}
        <div className={styles.aiChatInputWrapper}>
          <input
            type="text"
            className={styles.aiChatInput}
            placeholder={`Message ${currentModeInfo.label} Mentor...`}
            value={chatInput}
            onChange={(e) => setChatInput(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleSendMessage()}
          />
          <input type="file" ref={docInputRef} style={{ display: 'none' }} onChange={handleDocUpload} />
          <button className={styles.aiChatSendBtn} style={{ background: 'transparent', color: 'var(--text-muted)' }} onClick={() => docInputRef.current?.click()} disabled={isDocUploading} title="Attach Document">
            {isDocUploading ? <div className={styles.aiChatSpinner} /> : <Paperclip size={16} />}
          </button>
          {planFolder && (
            <button className={styles.aiChatSendBtn} style={{ background: 'transparent', color: 'var(--accent-base)', fontWeight: 600, padding: '0 8px', width: 'auto' }} onClick={onCommitPlan} disabled={isCommittingPlan} title="Accept & Commit Plan">
              {isCommittingPlan ? <div className={styles.aiChatSpinner} /> : <><Save size={16} /><span style={{ marginLeft: '4px', fontSize: '0.75rem' }}>Commit</span></>}
            </button>
          )}
          <button className={styles.aiChatSendBtn} onClick={handleSendMessage} disabled={isChatLoading || (!chatInput.trim() && !attachedDocsText)} style={{ background: currentModeInfo.color }}>
      ...
      {tasks.map((t, idx) => {
        ...
        return (
          <div key={idx} style={{ background: 'rgba(255,255,255,0.05)', padding: '8px', borderRadius: '6px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <a href={outlookUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.7rem', color: '#0078d4', textDecoration: 'none', background: 'rgba(0,120,212,0.1)', padding: '4px 8px', borderRadius: '4px' }}><Calendar size={12} /> Outlook</a>
              <a href={googleUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.7rem', color: '#ea4335', textDecoration: 'none', background: 'rgba(234,67,53,0.1)', padding: '4px 8px', borderRadius: '4px' }}><Calendar size={12} /> Google</a>
            </div>
          </div>
        );
      })}
